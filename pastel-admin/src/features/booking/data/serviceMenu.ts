...
const supportedManicureAddOns = ["chrome", "french", "ombre", "massage-scrub", "nail-design", "gel-color", "paraffin-hand-therapy", "russian-manicure"];
...
export function getServiceAddOns(groupId: ServiceGroupId, serviceSlug: string): ServiceAddOnOption[] {
  ...
  return (manicureAddOnsByService[slug] ?? [])
    .map((addOnSlug) => {
      ...
      if (slug === "pose" && addOnSlug === "russian-manicure") return { ...existingAddOn, name: "Classic Manicure" };
